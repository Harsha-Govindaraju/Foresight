    ...
    if (this.finalTranscript !== "") {
      ...
      Object.keys(commands).forEach(command => {
        if (typeof this.commands[command] === "function") {
          if (this.finalTranscript.indexOf(command) > -1) {
            if (this.finalTranscript[command.length] === undefined) {
              ...
              const param = this.finalTranscript
                .substring(command.length, this.finalTranscript.length)
                .trim();
          ...
          if (
            this.commands[command].fragments &&
            typeof this.commands[command].callback === "function"
          ) {
            const snippets = JSON.parse(
              JSON.stringify(this.commands[command].snippets)
            );
            const keywords = JSON.parse(
              JSON.stringify(this.commands[command].keywords)
            );
            const startsWidthKeyword = this.commands[command]
              .startsWidthKeyword;
            ...
            if (includes) {
              if (startsWidthKeyword) {
                transcriptFromTo = finalTranscript.substring(
                  finalTranscript.indexOf(snippets[0].toLowerCase())
                );
                const keyword = keywords[0];
                const keywordValue = finalTranscript.substring(
                  0,
                  finalTranscript.indexOf(snippets[0].toLowerCase())
                );
              ...
              for (let i = 0; i < snippets.length; i += 1) {
                ...
                transcript = transcript.replace(
                  new RegExp(`^${snippet.toLowerCase()}`, "gm"),
                  ""
                );
                let keywordValue;
                if (i + 1 < snippets.length) {
                  keywordValue = transcript.substring(
                    0,
                    transcript.indexOf(snippets[i + 1])
                  );
                } else {
              ...
              if (this.DEBUG) {
                console.info(
                  `calling command: "${command}" with params: `,
                  context
                );
              }
            }
...
AnyControl.prototype.addCommand = function addCommand(command, callback) {
  ...
  if (keywords) {
    const snippets = (command.split(/\${[a-zA-Z0-9_-]*}/gim) || [])
      .filter(n => n)
      .map(n => n.trim());
